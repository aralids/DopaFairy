...
def ODE_RHS(t, u, *, dose, half_life, t_admin):
    x = x_dose(t, dose=dose, half_life=half_life, t_admin=t_admin)

    ldopa, cda, vda, eda = u
    dldopa = V_TH(cda, eda) * C_TH(t) - V_AADC(ldopa)
    dcda = V_AADC(ldopa) - V_MAT(cda, vda) + V_DAT(eda) * (1-x) - k_cda*cda
    dvda = V_MAT(cda, vda) - fire(t) * vda
    deda = fire(t) * vda - V_DAT(eda) * (1-x) - V_CATAB(eda) * C_MAO(t) - k_eda*eda
